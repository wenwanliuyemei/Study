...
public class BinTreeUtils {
    ...
    private static BinTreeUtils binTreeUtils;

    public static BinTreeUtils getBinTreeUtils(){
        if (binTreeUtils==null){
            binTreeUtils=new BinTreeUtils();
        }
        return binTreeUtils;
    }
...
//        }
     ...
     * @return
     */
    public int minDepth(BinTreeNode root) {
     ...
     * @param node 当前结点
     */
    private void depth(BinTreeNode node) {
        if (node == null) {
            min = cur;
            return;
        }
        cur++; // 当前处理的层次加1
        // 如果是叶节点，并且路径比记录的最小还小
        if (node.leftChild == null && node.rightChild == null && cur < min) {
            min = cur; // 更新最小值
        }
        // 处理左子树
        if (node.leftChild != null) {
            depth(node.leftChild);
        }
        // 处理右子树
        if (node.rightChild != null) {
