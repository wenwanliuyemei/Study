...
public final class RSAUtils {
    ...
    public static String publicKey = "rsa_public_key.pem";
    public static String privateKey = "pkcs8_rsa_private_key.pem";
    ...
    public static String encode(Context context, String string) {

        try {
            // 从字符串中得到公钥
            //PublicKey publicKey = loadPublicKey(RSAUtils.PUCLIC_KEY);
            // 从文件中得到公钥
            InputStream inPublic = context.getResources().getAssets().open(RSAUtils.publicKey);
            PublicKey publicKey = loadPublicKey(inPublic);
            // 加密
            byte[] encryptByte = encryptData(string.getBytes(), publicKey);
            // 为了方便观察吧加密后的数据用base64加密转一下，要不然看起来是乱码,所以解密是也是要用Base64先转换
            string = Base64.encodeToString(encryptByte, Base64.NO_WRAP);
        } catch (IOException e) {
            e.printStackTrace();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static String decode(Context context, String strDecode) {
        try {
            // 从字符串中得到私钥
            //PrivateKey privateKeyKey = loadPrivateKey(RSAUtils.PRIVATE_KEY);
            // 从文件中得到私钥
            InputStream inPrivate = context.getResources().getAssets().open(RSAUtils.privateKey);
            PrivateKey privateKey = loadPrivateKey(inPrivate);
            // 因为RSA加密后的内容经Base64再加密转换了一下，所以先Base64解密回来再给RSA解密
            byte[] decryptByte = decryptData(Base64.decode(strDecode, Base64.NO_WRAP), privateKey);
            strDecode = new String(decryptByte);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return strDecode;
