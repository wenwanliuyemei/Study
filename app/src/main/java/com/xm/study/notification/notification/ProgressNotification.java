...
public class ProgressNotification extends MyNotification implements IProgressNotification {

    public int notifyId = 102;//Notification的ID
    private int progress = 0;//Notification的进度条数值

    public boolean isPause = false;//下载线程是否暂停
    public boolean isCustom = false;//通知栏内是否是自定义的
    public Boolean indeterminate = false;//true:为不确定样式的   false:确定样式
    ...
    private void initNotify() {
        mBuilder.setWhen(System.currentTimeMillis())// 通知产生的时间，会在通知信息里显示
                .setContentIntent(Utils.getDefalutIntent(mContext, 0))
     ...
     * 显示自定义的带进度条通知栏
     */
    public void showCustomProgressNotify(String status) {
        RemoteViews mRemoteViews = new RemoteViews(mContext.getPackageName(), R.layout.notification_custom_progress);
        mRemoteViews.setImageViewResource(R.id.iv_custom_progress_icon, R.mipmap.notification_custom_icon);
        ...
        if (progress >= 100 || downloadThread == null) {
            mRemoteViews.setProgressBar(R.id.pb_custom_progressbar, 0, 0, false);
            mRemoteViews.setViewVisibility(R.id.pb_custom_progressbar, View.GONE);
        } else {
            mRemoteViews.setProgressBar(R.id.pb_custom_progressbar, 100, progress, false);
            mRemoteViews.setViewVisibility(R.id.pb_custom_progressbar, View.VISIBLE);
        }
        ...
        mMyNotificationManager.mNotificationManager.notify(notifyId, nitify);
    }
